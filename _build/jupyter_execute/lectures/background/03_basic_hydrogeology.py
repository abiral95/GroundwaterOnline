...
# ```{note}
# As an example:
# For given data, determine the rate of outflow $Q_{out,sub}$ due to subsurface runoff for steady-state conditions
# ```
...
C = widgets.BoundedFloatText(
    ...
    max=1000,
    step=0.1,
    description=r'$M_C $(kg)',
    disabled=False
)

RA = widgets.BoundedFloatText(
    value=0.2,
    min=0,
    max=100,
    step=0.1,
    description=r'$R_A$ (day$^{-1}$)',
    disabled=False
)

RB = widgets.BoundedFloatText(
    value=0.2,
    min=0,
    max=100,
    step=0.1,
    description=r'$R_B$ (day$^{-1}$)',
    disabled=False
)
